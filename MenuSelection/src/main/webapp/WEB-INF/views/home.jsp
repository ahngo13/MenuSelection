		...
		<div class="row">
			<div class="col-sm-2">
				...
				<!-- <ul class="nav nav-pills flex-column">
					<li class="nav-item"><a class="nav-link active" href="#">products</a>
				</ul> -->
				<!-- <h3>members</h3> -->
				<ul class="nav nav-pills flex-column">
					<li class="nav-item"><a class="nav-link active bg-dark" href="#">members</a></li>
					<li class="nav-item"><a class="nav-link" href="#">siu</a></li>
					<li class="nav-item"><a class="nav-link" href="#">mincheol</a></li>
					<li class="nav-item"><a class="nav-link" href="#">mikyeong</a></li>
					<li class="nav-item"><a class="nav-link" href="#">sangmun</a></li>
					</li>
				</ul>
				<hr class="d-sm-none">
			</div>
			...
			<div id="map"
				style="width: 100%; height: 350px; margin: 5px 5px 0px 5px;"></div>
			<script src="/resources/js/jquery-3.4.1.min.js"></script>
			<script type="text/javascript"
				src="//dapi.kakao.com/v2/maps/sdk.js?appkey=8507bdcbfea943c9e828a2a8c91bd11a"></script>
			<script>
				...
				function addressSelect(y, x) {
					...
					jQuery
							.ajax({
								type : "get",
								//data: formData,
								url : '/kakao/category-search?x=' + x + '&y='
										+ y,
								success : function(data) {
									jQuery('#place-list').empty();
									jQuery('#place-list').append(data);

									let pageableCount = Number(jQuery(
											'#pageableCount').val());
									let positions = [];
									for (var i = 1; i < pageableCount + 1; i++) {
										var listName = jQuery('#listName' + i)
												.val();
										var listX = Number(jQuery('#listX' + i)
												.val());
										var listY = Number(jQuery('#listY' + i)
												.val());
				...
				function placeSelect(y, x, placeName) {
					...
					let locPosition = new kakao.maps.LatLng(y, x); // 마커가 표시될 위치를 geolocation으로 얻어온 좌표로 생성합니다
					let message = '<div style="padding:10px; text-align:center">'
							+ placeName + '</div>'; // 인포윈도우에 표시될 내용입니다
			...
			<div class="form-group" style="margin-bottom: 0px">
				<div>
					<input type="text" id="keyword" name="keyword" placeholder="주소 입력"
						onkeyup="javascript:enterkey();" class="form-control"
						style="width: 250px; float: left; margin: 5px 2px 5px 5px;" />
					<button class="btn btn-primary"
						onclick="javascript:addressSearch();"
						style="margin: 5px 5px 5px 2px; width: 100px">주소 검색</button>
				</div>
			</div>
